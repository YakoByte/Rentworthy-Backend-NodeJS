...
export default (app: Express) => {
    ...
    app.post('/location/create-location', UserAuth, async (req: AuthenticatedRequest, res: Response, next: NextFunction) => {
    ...
    app.post('/location/get-location', UserAuth, async (req: Request, res: Response, next: NextFunction) => {
        //validate admin from token
        // let admin = await (req);
        req.body._id = req.query._id
        console.log("req.body", req.body)
        try {
            const { data } = await service.getLocationById(req.body);
            return res.json(data);
        } catch (err) {
            next(err);
        }
    ...
    app.post('/location/update-location', UserAuth, async (req: AuthenticatedRequest, res: Response, next: NextFunction) => {
        //validate admin from token
        // let admin = await (req);
        let authUser: any = req.user
        req.body.userId = authUser._id;
    ...
    app.post('/location/delete-location', UserAuth, async (req: AuthenticatedRequest, res: Response, next: NextFunction) => {
        //validate admin from token
        // let admin = await (req);
        let authUser: any = req.user
        req.body.userId = authUser._id;
