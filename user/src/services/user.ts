...
class AdminService {
  ...
  async SignIn(userInputs: userLoginRequest) {
    ...
    try {
      ...
      // check role
      const checkRole: any = await this.repository.checkRole(
        userInputs.roleName,
      ...
      if (
        userInputs.bussinessType &&
        existingAdmin.bussinessType !== userInputs.bussinessType
      ) {
  ...
  async SignUp(userInputs: userSignRequest) {
    try {
      const newUser: any = await this.repository.CreateUser(userInputs);
  ...
  async ResetPassword(userInputs: userSetPasswordRequest) {
    try {
      ...
      let existingAdmin: any = await this.repository.FindUserById(
        userInputs._id
      );
      ...
      if (existingAdmin) {
        ...
        const updatedAdmin = await this.repository.UpdateUser(
          existingAdmin._id,
          {
            password,
          }
  ...
  async SocialSignUp(userInputs: socialUserSignRequest) {
    try {
      const newUser: any = await this.repository.SocialCreateUser(userInputs);
      return FormateData(newUser);
    } catch (err: any) {
      console.log("err", err);
      throw new APIError("Data Not found", err);
  ...
  async SocialSignIn(userInputs: socialUserLoginRequest) {
    ...
    try {
      ...
      if (
        userInputs.loginType &&
        existingAdmin.loginType === userInputs.loginType
      ) {
        const token = await GenerateSignature({
          email: existingAdmin.email,
          _id: existingAdmin._id,
